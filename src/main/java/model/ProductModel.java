...
public class ProductModel {
	...
	public synchronized Collection<ProductBean> doRetrieveAll(String where) throws SQLException {
	    ...
	    try {
	        connection = DriverManagerConnectionPool.getConnection();
	        preparedStatement = connection.prepareStatement(selectSQL);
	        preparedStatement.setString(1, where);  // Setta il parametro della query
	        ...
	        while (rs.next()) {
	            ...
	            // Usa la stessa connessione per la seconda query
	        ...
	        // Assicurati di chiudere le risorse in un blocco finally per garantire che siano sempre chiuse
	        if (preparedStatement != null) {
	            try {
	                preparedStatement.close();
